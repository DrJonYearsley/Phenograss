...
#calculate mean Biomass for every variety in every chamber
...
stats.Biomass=function(no_col=1, names=c("cut 2")){
  ...
  stats_Biomass=aggregate(Biomass[,5], by=list(Varietys=Biomass$Variety, Chamber=Biomass$Chamber), mean)
  names(stats_Biomass)[3]="cut 1"
  for (i in cols) {
    dummy_stats=aggregate(Biomass[,i], by=list(Varietys=Biomass$Variety, Chamber=Biomass$Chamber), mean)
    col=i-2
...
get.Biomass.time.series=function(Chambernr=1){
  Chamber=stats_Biomass[stats_Biomass$Chamber==Chambernr,]
  Chamber_dummy=rbind(Chamber$`cut 1`, Chamber$`cut 2`)
  colnames(Chamber_dummy)=Chamber$Varietys
  Chamber_dummy=cbind(Chamber_dummy,week=c(1,2))
  ...
  ggplot(df, aes(x = week, y = value)) + #basic plot 
    ...
    labs(title=paste("Chamber",Chambernr,"Lolium perenne Biomass 27. Feb 2020")) #title of plot #subtitle of plot
}
get.Biomass.time.series(Chambernr=1)
...
save.Biomass.plot=function(){
  for (i in Chambers){
    ...
    get.Biomass.time.series(Chambernr=i)
...
mean_Biomass_chamber3=aggregate(stats_Biomass_30$avg_Biomass_30, 
                               by = list(Chamber=stats_Biomass_30$Chamber), mean) #calculate mean per chamber
str(mean_Biomass_chamber) #check data
H_lines3=geom_hline(data=mean_Biomass_chamber3, aes(yintercept=x), color="red") #create line layer for mean
...
stats_Biomass_dummy=data.frame(Dummy=rep(1,112), "Biomass"=stats_Biomass$avg_Biomass_17, "Varieties"=stats_Biomass$Varietys, "Chamber"=stats_Biomass$Chamber)
stats_Biomass_dummy[57:112,2]=stats_Biomass$Growth
stats_Biomass_dummy[57:112,3]=stats_Biomass$Varietys
stats_Biomass_dummy[57:112,4]=stats_Biomass$Chamber
...
H_lines=geom_hline(data=mean_Biomass_chamber, aes(yintercept=x), color="grey") #create line layer for mean
H_lines2=geom_hline(data=mean_Biomass_chamber2, aes(yintercept=x), color="black") #create line layer for mean
H_plot3+H_bars3+facet_grid(.~Chamber)+
